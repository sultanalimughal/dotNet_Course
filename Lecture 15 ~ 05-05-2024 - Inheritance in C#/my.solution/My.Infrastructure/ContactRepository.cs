...
namespace My.Infrastructure
{
    public struct ContactRepository
    {
        ...
        {
            Contact contact = new WorkContact( "Qalb" , "qalb@gmail.com" , "DotNet Developer")
            {
                Address = "abc",
        ...
        public void ContactGetAll()
        {
            if (_arrayIndex > 0)
            {
                ...
                {
                    var workContact =_contacts[i] as WorkContact;
                    Console.WriteLine($@"---------- Value at {i + 1} ------------------");
                    Console.WriteLine($@"Name : {workContact.Name} , Email : {workContact.Email} , Phone : {workContact.Phone} , Address : {workContact.Address} , Designation : {workContact.Designation}");
                    Console.WriteLine("------------------------------------------------");
                }
            }
            else
